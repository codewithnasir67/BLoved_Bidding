...
const CreateProduct = () => {
  ...
  useEffect(() => {
    ...
    if (!seller && storedSeller) {
      try {
        const parsedSeller = JSON.parse(storedSeller);
        if (!parsedSeller._id || parsedSeller._id === 'null' || parsedSeller._id === 'undefined') {
    ...
    if (!seller && !storedSeller) {
      toast.error("Please login to your seller account first!");
      navigate('/shop-login');
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const storedSeller = localStorage.getItem("seller");
    const currentSeller = seller || (storedSeller ? JSON.parse(storedSeller) : null);
    ...
    if (!name) { toast.error("Please enter product name!"); return; }
    if (!description) { toast.error("Please enter product description!"); return; }
    if (!category) { toast.error("Please select product category!"); return; }
    if (!discountPrice) { toast.error("Please enter product price!"); return; }
    if (!stock) { toast.error("Please enter product stock!"); return; }
    if (images.length === 0) { toast.error("Please add at least one product image!"); return; }


    try {
      const newForm = new FormData();
      // Add shop ID - ensure it's a valid ObjectId string
      if (!currentSeller._id || currentSeller._id === 'null' || currentSeller._id === 'undefined') {
        toast.error("Invalid seller ID. Please login again!");
        navigate('/shop-login');
        return;
      }

      const shopId = currentSeller._id.toString();
      ...
      if (isAuction) {
        if (!startingPrice) { toast.error("Please enter starting price!"); return; }
        if (!incrementValue) { toast.error("Please enter increment value!"); return; }

        newForm.append("startingPrice", startingPrice);
        newForm.append("incrementValue", incrementValue);

        const endTime = new Date();
        endTime.setHours(endTime.getHours() + parseInt(biddingDuration));
        newForm.append("auctionEndTime", endTime.toISOString());
  ...
  // Image sidebar to confirm selection
      ...
      <div className="max-w-4xl w-full space-y-8 bg-white p-10 rounded-2xl shadow-xl">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Fill in the details below to list your item for sale or auction.
          </p>
        </div>
                ...
                <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                  <div>
                    ...
                    <select
                      ...
                      className="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-brand-teal focus:border-brand-teal sm:text-sm"
                    >
